  ...
  const setCardContent = (character, data) => {
    ...
    if (data.hasOwnProperty('locations')) {
      presets.dynamic.origin = {
        ...
        dimension:
          data.locations?.[originName] && data.locations?.[originName]['dimension']
            ? capitalizeFirstLetter(data['locations'][originName]['dimension'])
            : 'Unknown',
        type:
          data.locations?.[originName] && data.locations?.[originName]['type']
            ? capitalizeFirstLetter(data['locations'][originName]['type'])
            : 'Unknown',
        'amount of residents':
      ...
      presets.dynamic.location = {
        ...
        dimension:
          data.locations?.[locationName] && data.locations?.[locationName]['dimension']
            ? capitalizeFirstLetter(data['locations'][locationName]['dimension'])
            : 'Unknown',
        type:
          data.locations?.[locationName] && data.locations?.[locationName]['type']
            ? capitalizeFirstLetter(data['locations'][locationName]['type'])
            : 'Unknown',
        'amount of residents':
    ...
    if (data.hasOwnProperty('episodes')) {
      presets.dynamic.episodes = data['episodes']
        ? getEpisodesInfo(getEpisodesIndexes(episodes), data['episodes'])
        : [];
    }
    ...
    <section className={`${parentClass ? parentClass : ''} character-list`}>
      {characters.length ? (
        characters.map((character, index) => (
          <CharacterCard
            character={character}
            content={setCardContent(character, content)}
            parentClass="characters-list__character-item"
            key={index}
          />
        ))
      ) : (
